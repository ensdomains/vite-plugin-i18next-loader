...
describe('pathOverride', () => {
  for (const type of ['yaml', 'json']) {
    const appLocalesDir = path.join(
      __dirname,
      `./data/override-app-${type}/locales`,
    )
    describe(type, () => {
      ...
      it.concurrent('should merge appLocales over any libraries', async () => {
        const load = factory({
          paths: [
            path.join(appLocalesDir, '../node_modules/lib/locales'),
            appLocalesDir,
          ],
        }).load
        const res = (load as any).call(
        ...
        expect(resStore['zh-cn'].main.sub.test).toStrictEqual(
          'lib sub.test zh-cn',
        )
        expect(resStore['zh-cn'].main.sub.subsub.test).toStrictEqual(
          'lib sub.subsub.test zh-cn',
        )
        expect(resStore['zh-cn'].main.sub.slug).toStrictEqual(
          'app sub.slug zh-cn',
        )
        expect(resStore['zh-cn'].main.sub.subsub.slugslug).toStrictEqual(
          'app sub.subsub.slugsub zh-cn',
